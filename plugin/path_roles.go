...
// for a Vault role to access and call the Pwmgr
// token endpoints
...
func (b *pwmgrBackend) pathRolesList(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
...
func (b *pwmgrBackend) pathRolesRead(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
...
func (b *pwmgrBackend) pathRolesWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
...
func (b *pwmgrBackend) pathRolesDelete(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	err := req.Storage.Delete(ctx, "role/"+d.Get("name").(string))
	if err != nil {
		return nil, fmt.Errorf("error deleting pwmgr role: %w", err)
	}
...
func setRole(ctx context.Context, s logical.Storage, name string, roleEntry *pwmgrRoleEntry) error {
...
func (b *pwmgrBackend) getRole(ctx context.Context, s logical.Storage, name string) (*pwmgrRoleEntry, error) {
	if name == "" {
...
const (
	pathRoleHelpSynopsis    = `Manages the Vault role for generating Pwmgr tokens.`
	pathRoleHelpDescription = `
This path allows you to read and write roles used to generate Pwmgr tokens.
You can configure a role to manage a user's token by setting the username field.
`

	pathRoleListHelpSynopsis    = `List the existing roles in Pwmgr backend`
