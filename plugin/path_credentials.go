...
// pathCredentialsRead creates a new Pwmgr token each time it is called if a
// role exists.
func (b *pwmgrBackend) pathCredentialsRead(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
...
// createUserCreds creates a new Pwmgr token to store into the Vault backend, generates
// a response with the secrets information, and checks the TTL and MaxTTL attributes.
func (b *pwmgrBackend) createUserCreds(ctx context.Context, req *logical.Request, role *pwmgrRoleEntry) (*logical.Response, error) {
	...
	resp := b.Secret(pwmgrTokenType).Response(map[string]interface{}{
		"token":    token.Token,
...
// createToken uses the Pwmgr client to sign in and get a new token
func (b *pwmgrBackend) createToken(ctx context.Context, s logical.Storage, roleEntry *pwmgrRoleEntry) (*pwmgrToken, error) {
	client, err := b.getClient(ctx, s)
	if err != nil {
		return nil, err
	}

	var token *pwmgrToken
	...
	if token == nil {
		return nil, errors.New("error creating Pwmgr token")
	}
